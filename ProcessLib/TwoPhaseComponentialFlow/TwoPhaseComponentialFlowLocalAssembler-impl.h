                ...
                std::vector<double>& local_b_data)
        {
            ...
            for (unsigned ip = 0; ip < n_integration_points; ip++)
            {
                ...
                double const x_wet_h2o =
                    ...
                    //1 - X_L_co2_gp - X_L_c_gp - X_L_h_gp - x_wet_air;
                    //
                ...
                pg_int_pt - 1e-6, X1_int_pt, X2_int_pt, X3_int_pt, P_sat_gp,
                kelvin_term)) / 2 / 1e-6;
                double TEST = std::abs(d_x_nonwet_h2o_d_pg - d_x_nonwet_h2o_d_pg_test)
                    / d_x_nonwet_h2o_d_pg;*/
                ...
                pg_int_pt, X1_int_pt - 1e-6, X2_int_pt, X3_int_pt, P_sat_gp,
                kelvin_term)) / 2 / 1e-6;
                double TEST1 = std::abs(d_x_nonwet_h2o_d_x1 - d_x_nonwet_h2o_d_x1_test)
                ...
                pg_int_pt, X1_int_pt, X2_int_pt - 1e-6, X3_int_pt, P_sat_gp,
                kelvin_term)) / 2 / 1e-6;
                double TEST2 = std::abs(d_x_nonwet_h2o_d_x2 - d_x_nonwet_h2o_d_x2_test)
               ...
               double d_x_nonwet_h2o_d_x3_test = (get_x_nonwet_h2o(
                ...
                double TESTpc = std::abs(d_x_nonwet_h2o_d_kelvin - d_x_nonwet_h2o_d_kelvin_test)
...
//                double M_organic_slow_co2_ini =  _amount_organic_waste_polystyrene[ip]; // amount from which gas is produced
                ...
                // air
                mass_mat_coeff(3, 0) =
                    porosity * ((1 - Sw) *d_rho_mol_nonwet_d_pg +
                ...
                mass_mat_coeff(3, 3) =
                    porosity * (0);
                mass_mat_coeff(3, 4) =
                    porosity * (rho_mol_nonwet *dSgdPC -
                        rho_mol_wet * dSgdPC);
                ...
                mass_mat_coeff(4, 4) =
                    porosity * (rho_mol_nonwet * x_nonwet_h2o*dSgdPC+
                        rho_mol_nonwet* d_x_nonwet_h2o_d_pc*(1 - Sw)
                        - rho_mol_water * dSgdPC);
                //-------------debugging------------------------
                ...
                Kairx1.noalias() += sm.N.transpose()*(darcy_volumetric_flux_gas_phase.transpose()
                    ...
                    * rho_mol_wet*pg_int_pt*d_x_nonwet_air_d_x1 / Hen_L_air
                    + darcy_volumetric_flux_liquid_phase.transpose()
                    * x_wet_air*d_rho_mol_wet_d_x1
                    )*integration_factor*sm.dNdx;
                Kairx2.noalias() += sm.N.transpose()*(darcy_volumetric_flux_gas_phase.transpose()
                    *(rho_mol_nonwet*d_x_nonwet_air_d_x2)
                    + darcy_volumetric_flux_liquid_phase.transpose()
                    * rho_mol_wet*pg_int_pt*d_x_nonwet_air_d_x2 / Hen_L_air
                    + darcy_volumetric_flux_liquid_phase.transpose()
                    * x_wet_air*d_rho_mol_wet_d_x2)
                    *integration_factor*sm.dNdx;
                Kairx3.noalias() += sm.N.transpose()*(darcy_volumetric_flux_gas_phase.transpose()
                    *(rho_mol_nonwet*d_x_nonwet_air_d_x3)
                    + darcy_volumetric_flux_liquid_phase.transpose()
                    * rho_mol_wet * pg_int_pt*d_x_nonwet_air_d_x3 / Hen_L_air
                    + darcy_volumetric_flux_liquid_phase.transpose()
                    * x_wet_air*d_rho_mol_wet_d_x3)
                    *integration_factor*sm.dNdx;
                Kairpc.noalias() += sm.N.transpose()*(darcy_volumetric_flux_gas_phase.transpose()
                ...
                */
                ...
                Kh2opg.noalias() += sm.N.transpose()*darcy_volumetric_flux_gas_phase.transpose()
                    *(rho_mol_nonwet * d_x_nonwet_h2o_d_pg)
                    *integration_factor*sm.dNdx;
...
//                       double& M_organic_fast_co2_ini = _ip_data[ip].amount_organic_waste_prev_cellulose; // amount from which gas is produced
  ...
  //                   double& M_organic_slow_co2_ini = _ip_data[ip].amount_organic_waste_prev_polystyrene; // amount from which gas is produced
                       ...
                       _ip_data[ip].amount_organic_waste_polystyrene=_amount_organic_waste_polystyrene[ip];
                        ...
                        F_vec_coeff(4) -= Q_organic_fast_co2 * 1/3;
                        //total components
                        F_vec_coeff(3) += Q_steel_waste_matrix
                            + (Q_organic_slow_co2 * 5 / 3)
                            + Q_organic_slow_co2
                            + Q_organic_fast_co2
                            + Q_organic_fast_co2
                            - Q_steel_waste_matrix
                            - Q_organic_slow_co2 * 2
                            - Q_organic_fast_co2 * 1 / 3;
                        ...
                        pos.getElementID().get()) == 0)//backfill, cement and concrete
                    {
                        ...
                        // update the current fluid volume
                        fluid_volume_backfill= bi_interpolation(
                            rho_mol_sio2_wet_backfill,
                            rho_mol_co2_cumul_total_backfill, _fluid_volume_suppt_pnt_backfill);
                        ...
                        // total components
                        F_vec_coeff(3) += (fluid_volume_rate*rho_mol_water)
                            - rho_mol_co2_kinetic_rate_backfill
                            + (_ip_data[ip].porosity_prev_backfill - porosity2)
                            *(rho_mol_water*_saturation[ip] + rho_mol_nonwet * x_nonwet_h2o*(1 - _saturation[ip]))
                            / dt;
                        // account for change in water content due to porosity change ...we loose mass if porosity gets smaller and saturation is assumed to be constant
                        // fluid_flux [m^3/a] = (porosity_value_old[ip]-porosity_value_new[ip])*saturation_value[i] / dt
                        // positive values (porosity gets smaller): we add the lost water during next time step as source term
                        ...
                        double const gas_hydrogen_variation_rate_porosity=
                            (_ip_data[ip].porosity_prev_backfill - porosity2) 
                            * (_saturation[ip] * rho_mol_wet*X_L_h_gp
                                + (1 - _saturation[ip])*rho_mol_nonwet*X1_int_pt)/ dt;
                        F_vec_coeff(0) += gas_hydrogen_variation_rate_porosity;
                        double const gas_methane_variation_rate_porosity =
                            (_ip_data[ip].porosity_prev_backfill - porosity2)
                            * (_saturation[ip] * rho_mol_wet*X_L_c_gp
                                + (1 - _saturation[ip])*rho_mol_nonwet*X2_int_pt) / dt;
                        F_vec_coeff(1) += gas_methane_variation_rate_porosity;

                        double const gas_co2_variation_rate_porosity =
                            (_ip_data[ip].porosity_prev_backfill - porosity2) 
